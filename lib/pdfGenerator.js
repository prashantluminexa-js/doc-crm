...
export async function generateDocumentPDF({ documentData, client, settings }) {
  ...
  function drawHeader(pageIndex = 0) {
    if (logo) {
      doc.addImage(logo, "PNG", margin, 16, 38, 5.4, undefined, "FAST");
    }
    ...
    doc.text(`${docLabel}# ${documentData.documentNumber || "-"}`, right, 33.5, {
      align: "right",
    });

    if (isQuotation) {
      doc.text("Valid till 1 Month", right, 39, { align: "right" });

      doc.setFont("helvetica", "bold");
      doc.setFontSize(7.5);
      doc.text(
        isOptionsQuotation ? "Options Quotation" : "Total Amount Quotation",
        right,
        44,
        { align: "right" }
      );
  ...
  function drawFooter() {
    ...
    doc.setFont("helvetica", "bold");
    doc.text(companyName, footerTextX, footerY + 4.2);
  }
